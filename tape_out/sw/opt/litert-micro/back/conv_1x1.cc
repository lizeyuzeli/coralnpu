...
#define CONV_1x1_LOAD_INPUT()                                       \
  asm("vsetvli zero, %[vl], e16, m2, ta, ma;"                       \
      "vle8.v v30, %[input_ptr];"                                   \
      "vsext.vf2 %[in_vec], v30;"                                   \
      "vadd.vx %[in_vec], %[in_vec], %[input_offset];"              \
      : [in_vec] "=vr" (in_vec)                                     \
      : [vl] "r"(vl), [input_ptr] "A"(*in_ptr_chunk),               \
        [input_offset] "r"(input_offset)                            \
      : "v30", "v31", "vl", "vtype");                               

#define CONV_1x1_MAC(fil, idx)                                      \
  asm("vsext.vf2 v30, %[filter];"                                   \
      "vwmul.vv %[mul_vec], %[in_vec], v30;"                        \
      : [mul_vec] "+vr"(mul_vec)                                    \
      : [filter] "vr"(fil), [in_vec] "vr"(in_vec)                   \
      : "v30", "v31");                                              \
...
static inline void CONV_1x1_MAC_REM(int32_t* accs_buf, int idx, int block_base_ptr, size_t vl,
                                    vint16m2_t in_vec, vint8m1_t fil, vint32m4_t mul_vec) {
  asm("vsext.vf2 v30, %[filter];"                                   
      "vwmul.vv %[mul_vec], %[in_vec], v30;"                        
      : [mul_vec] "+vr"(mul_vec)                                    
      : [filter] "vr"(fil), [in_vec] "vr"(in_vec)                   
      : "v18", "v19", "v30", "v31");                                
      int32_t acc_val = __riscv_vmv_x_s_i32m1_i32(
                        __riscv_vredsum_vs_i32m4_i32m1(mul_vec, 
                        __riscv_vmv_v_x_i32m1(0, 1), vl));  
  accs_buf[block_base_ptr + idx] += acc_val; 
}
...
void Conv_1x1_Tiled16(
  ...
  for (int out_channel = 0; out_channel < output_depth; out_channel += OUT_BLOCK_SIZE) {
    const int this_block_size = std::min(OUT_BLOCK_SIZE, output_depth - out_channel);
    // Process input depth in chunks of IN_CHUNK_SIZE (last chunk may be smaller)
    for (int in_channel = 0; in_channel < input_depth; in_channel += IN_CHUNK_SIZE) {
      const int this_chunk_size = std::min(IN_CHUNK_SIZE, input_depth - in_channel);
      // set vl for this chunk (number of 8-bit elements)
      size_t vl = __riscv_vsetvl_e8m1(this_chunk_size);
      const bool full_tile = (this_block_size == OUT_BLOCK_SIZE) && (this_chunk_size == IN_CHUNK_SIZE);
      ...
      register vint8m1_t fil_vec0 __asm__("v1");
      register vint8m1_t fil_vec1 __asm__("v2");
      register vint8m1_t fil_vec2 __asm__("v3");
      register vint8m1_t fil_vec3 __asm__("v4");
      register vint8m1_t fil_vec4 __asm__("v5");
      register vint8m1_t fil_vec5 __asm__("v6");
      register vint8m1_t fil_vec6 __asm__("v7");
      register vint8m1_t fil_vec7 __asm__("v8");
      register vint8m1_t fil_vec8 __asm__("v9");
      register vint8m1_t fil_vec9 __asm__("v10");
      register vint8m1_t fil_vec10 __asm__("v11");
      register vint8m1_t fil_vec11 __asm__("v12");
      register vint8m1_t fil_vec12 __asm__("v13");
      register vint8m1_t fil_vec13 __asm__("v14");
      register vint8m1_t fil_vec14 __asm__("v15");
      register vint8m1_t fil_vec15 __asm__("v16");

      fil_vec0 = __riscv_vle8_v_i8m1(filter_base_ptr, vl);
      fil_vec1 = __riscv_vle8_v_i8m1(filter_base_ptr + 1 * filter_col_stride, vl);
      fil_vec2 = __riscv_vle8_v_i8m1(filter_base_ptr + 2 * filter_col_stride, vl);
      fil_vec3 = __riscv_vle8_v_i8m1(filter_base_ptr + 3 * filter_col_stride, vl);
      fil_vec4 = __riscv_vle8_v_i8m1(filter_base_ptr + 4 * filter_col_stride, vl);
      fil_vec5 = __riscv_vle8_v_i8m1(filter_base_ptr + 5 * filter_col_stride, vl);
      fil_vec6 = __riscv_vle8_v_i8m1(filter_base_ptr + 6 * filter_col_stride, vl);
      fil_vec7 = __riscv_vle8_v_i8m1(filter_base_ptr + 7 * filter_col_stride, vl);
      fil_vec8 = __riscv_vle8_v_i8m1(filter_base_ptr + 8 * filter_col_stride, vl);
      fil_vec9 = __riscv_vle8_v_i8m1(filter_base_ptr + 9 * filter_col_stride, vl);
      fil_vec10 = __riscv_vle8_v_i8m1(filter_base_ptr + 10 * filter_col_stride, vl);
      fil_vec11 = __riscv_vle8_v_i8m1(filter_base_ptr + 11 * filter_col_stride, vl);
      fil_vec12 = __riscv_vle8_v_i8m1(filter_base_ptr + 12 * filter_col_stride, vl);
      fil_vec13 = __riscv_vle8_v_i8m1(filter_base_ptr + 13 * filter_col_stride, vl);
      fil_vec14 = __riscv_vle8_v_i8m1(filter_base_ptr + 14 * filter_col_stride, vl);
      fil_vec15 = __riscv_vle8_v_i8m1(filter_base_ptr + 15 * filter_col_stride, vl);
      for (int batch = 0; batch < batches; ++batch) {
        for (int out_y = 0; out_y < output_height; ++out_y) {
          const int in_y_origin = out_y * stride_height - pad_height;
          for (int out_x = 0; out_x < output_width; ++out_x) {
            const int in_x_origin = out_x * stride_width - pad_width;
            // If input spatial for this output is OOB, contribution is zero.
            if (in_y_origin < 0 || in_y_origin >= input_height || in_x_origin < 0 || in_x_origin >= input_width) {
              continue;
            }
            const int8_t* in_ptr_chunk = input_data + Offset(input_shape, batch, in_y_origin, in_x_origin, in_channel);
        ...
        vint8m1_t fil_vecs[OUT_BLOCK_SIZE];
        for (int j = 0; j < this_block_size; ++j) {
          const int out_channel_rem = out_channel + j;
          const int8_t* fil_ptr = filter_data + Offset(filter_shape, out_channel_rem, 0, 0, in_channel);
          fil_vecs[j] = __riscv_vle8_v_i8m1(fil_ptr, vl);
        }
        for (int batch = 0; batch < batches; ++batch) {
        for (int out_y = 0; out_y < output_height; ++out_y) {
          const int in_y_origin = out_y * stride_height - pad_height;
          for (int out_x = 0; out_x < output_width; ++out_x) {
            ...
            CONV_1x1_LOAD_INPUT();
            for (int j = 0; j < this_block_size; ++j) {
              CONV_1x1_MAC_REM(accs_buf, j, block_base_ptr, vl, in_vec, fil_vecs[j], mul_vec);
            }
          }    // out_x
        }      // out_y
      }        // batch
      ...
      for (int batch = 0; batch < batches; ++batch) {
        for (int out_y = 0; out_y < output_height; ++out_y) {
          ...
          for (int out_x = 0; out_x < output_width; ++out_x) {
            ...
            for (int j = 0; j < this_block_size; ++j) {
              fil_vec_ext = __riscv_vsext_vf2_i16m2(fil_vecs[j], vl);                  
              mul_vec = __riscv_vwmul_vv_i32m4(in_vec_ext, fil_vec_ext, vl);  
              acc_val = __riscv_vmv_x_s_i32m1_i32(                                  
                        __riscv_vredsum_vs_i32m4_i32m1(mul_vec,                     
                        __riscv_vmv_v_x_i32m1(0, 1), vl));                          
              accs_buf[block_base_ptr + j] += acc_val;
            }
          }    // out_x
        }      // out_y
      }        // batch
