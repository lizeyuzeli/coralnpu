...
// Inner AXI kernel: the original CoreAxi logic, now renamed. The flat verification
// top (class CoreAxi) lives in CoreAxi.scala and wraps this kernel
// + LinkAdapter via CoreChip so all existing test targets keep their top name.
// Boot address selection is reduced to a 1-bit `boot_sel` input (0 -> 0x0,
// 1 -> 0x10000000) to minimise inter-die link width and keep the surrounding
// tape-out wrapper interface narrow.
class CoreAxiKernel(p: Parameters, coreModuleName: String) extends RawModule {
  override val desiredName = coreModuleName + "AxiKernel"
  ...
  val io = IO(new Bundle {
    ...
    val wfi = Output(Bool())
    val irq = Input(Bool())
    // 1-bit boot-address select (0 -> 0x0, 1 -> 0x10000000), see plan §2.1.
  ...
  withClockAndReset(rst_sync.io.clk_o, global_reset) {
    ...
    csr.io.bootAddr := Mux(io.boot_sel, 0x10000000.U(p.fetchAddrBits.W), 0.U(p.fetchAddrBits.W))
