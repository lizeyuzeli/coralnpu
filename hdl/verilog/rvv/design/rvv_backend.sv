...
`ifdef FAULT_TOLERANT_ON
  ...
  // Width = strict max over ALL replicated RS payloads (ALU/MUL/DIV + FALU when
  // ZVE32F_ON), so a wider struct can never be silently truncated on capture.
  `ifdef ZVE32F_ON
    localparam int FT_RS_W = `MAX4($bits(ALU_RS_t),$bits(MUL_RS_t),$bits(DIV_RS_t),$bits(FALU_RS_t));
  `else
    localparam int FT_RS_W = `MAX4($bits(ALU_RS_t),$bits(MUL_RS_t),$bits(DIV_RS_t),0);
  `endif
    logic [FT_RS_W-1:0]                   replay_mem [`ROB_DEPTH-1:0];
  // reinject command bus: [unit][port], unit 0=ALU,1=MUL/MAC,2=DIV,3=FALU
  ...
  `ifdef ZVE32F_ON
    logic         [`NUM_DP_UOP-1:0]       falu_push_ft;
    FALU_RS_t     [`NUM_DP_UOP-1:0]       falu_datain_ft;
  `endif
`endif
`ifdef ZVE32F_ON
    // FALU_RS
...
`ifdef FAULT_TOLERANT_ON
  ...
  // per port, so reinject only ever lands on idle ports).
    ...
    generate
      ...
      `ifdef ZVE32F_ON
        assign reinject_freeslot_dp2rob[3][ft_p] = rs_ready_falu2dp[ft_p] & ~rs_valid_dp2falu[ft_p];
      `else
        assign reinject_freeslot_dp2rob[3][ft_p] = 1'b0; // no FALU RS when ZVE32F off
      `endif
      end
    endgenerate
  ...
  // overwritten on the next push and the ROB pending counter gates replay.
    always_ff @(posedge clk) begin
      ...
      `ifdef ZVE32F_ON
        else if (rs_valid_dp2falu[i] & uop_dp2rob[i].is_ft)
          replay_mem[rs_dp2falu[i].rob_entry] <= FT_RS_W'(rs_dp2falu[i]);
      `endif
      end
    end
  ...
  // payload, so replaying it preserves the original ROB slot (INV-2).
    always_comb begin
      ...
      `ifdef ZVE32F_ON
        // FALU (incl. FCVT/FCMP/FNCMP/FTBL/sqrt7/rec7)
        if (reinject_valid_rob2dp[3][p]) begin
          falu_push_ft[p]   = 1'b1;
          falu_datain_ft[p] = FALU_RS_t'(replay_mem[reinject_entry_rob2dp[3][p]]);
        end else begin
          falu_push_ft[p]   = rs_valid_dp2falu[p];
          falu_datain_ft[p] = rs_dp2falu[p];
        end
      `endif
      end
    end
