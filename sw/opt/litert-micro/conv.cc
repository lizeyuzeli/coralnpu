...
void ConvPerChannel(const ConvParams &params, const OpDataConvCustom &data,
  ...
  if (filter_height == 1 && filter_width == 1 &&
    ...
    Conv2D_Generic(params, data, input_shape, input_data, filter_shape, filter_data_copy.get(),
                   bias_data_copy.get(), output_shape, output_data,
...
TfLiteStatus ConvPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  // Generic repacked weights ([ky][kx][ic][oc]) are used by the optimized 1x1 / 3x3 / 4x4 kernels
  // and by the Conv2D_Generic OC-vectorized fallback for arbitrary (kH, kW) (e.g. KWS-MicroNet's
  // 10x4 stem). 1x1 does not need the padded tile buffer (no spatial accesses); 3x3 and 4x4 do; the
  // generic fallback uses bounds-checked direct loads and also doesn't need it.
  ...
  if (wants_generic_repack) {
    ...
    // Padded tile buffer is only used by Conv2D_3x3 and Conv2D_4x4. 1x1 has no spatial accesses;
    // the generic OC-vectorized fallback does its own bounds checks and doesn't read the padded
    // tile.
    const bool wants_padded_tile =
        (filter_height == 3 && filter_width == 3) || (filter_height == 4 && filter_width == 4);
