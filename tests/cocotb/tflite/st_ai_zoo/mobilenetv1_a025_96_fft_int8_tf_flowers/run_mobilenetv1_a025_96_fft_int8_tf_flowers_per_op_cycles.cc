...
// Per-op cycle profiling variant of the 96x96 FFT runner.
//
// Wires a custom MicroProfilerInterface into MicroInterpreter that records
// mcycle deltas around every op invocation. After Invoke, the recorded
// (tag, cycles) pairs are exposed as globals for the cocotb side to dump
// as a CSV under tests/.../reports/.
//
// The compute path is identical to the clean runner -- this file only adds
// the profiler plumbing.
...
constexpr int kInputElems = 1 * 96 * 96 * 3;
constexpr int kInputBytes = kInputElems * 1;
constexpr int kOutputElems = 1 * 5;
constexpr int kOutputBytes = kOutputElems * 4;

// Per-op profile storage exposed to cocotb. kMaxOps must comfortably fit the
// model's op count (32 for this model); kTagBytes must match the matching
// constant in the cocotb side.
...
// --- Per-op cycle profile, read by cocotb after run_to_halt.
uint32_t op_profile_count __attribute__((section(".data"))) = 0;
uint32_t op_profile_cycles[kMaxOps] __attribute__((section(".data"))) = {};
// Flat (kMaxOps * kTagBytes) byte buffer; each kTagBytes-byte slot holds a
// NUL-terminated op tag string (e.g. "CONV_2D").
...
uint32_t CycleProfiler::BeginEvent(const char* tag) {
  if (op_profile_count >= kMaxOps) {
    // Overflow -- reuse the last slot; cocotb side will see saturated count.
    return kMaxOps - 1;
  }
  uint32_t h = op_profile_count;
  // Copy tag into the fixed-size flat buffer.
