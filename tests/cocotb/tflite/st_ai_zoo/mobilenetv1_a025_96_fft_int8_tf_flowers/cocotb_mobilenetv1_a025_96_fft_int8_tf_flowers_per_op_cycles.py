...
"""Cocotb test that runs the 96x96 FFT MobileNet-V1 INT8 model with a per-op
MicroProfiler hooked into the interpreter and dumps a per-op cycle/time
report as a bazel undeclared test output (`op_cycles.csv`).
...
Pass criteria: inference succeeds and at least one non-zero per-op cycle
sample is captured. The actual cycle numbers are reported as artefacts only,
not asserted -- this lets the file double as a profiling tool that won't
flap the CI on micro-optimisation deltas.

Output CSV columns:
    idx, op_name, cycles, time_ns

`time_ns` is computed from the simulator clock period (see
`CoreMiniAxiInterface.clock_ns`, default 1.25 ns @ 800 MHz).
"""
...
_ELF = "run_mobilenetv1_a025_96_fft_int8_tf_flowers_per_op_cycles_binary.elf"
_INPUT_NPY = "test_data/input_0.npy"

# Must match kMaxOps / kTagBytes in
# run_mobilenetv1_a025_96_fft_int8_tf_flowers_per_op_cycles.cc.
_MAX_OPS = 64
_TAG_BYTES = 24

def _pick_reports_dir() -> str:
    """Return the bazel-managed undeclared-outputs dir, or /tmp as a fallback.

    Under `bazel test` the runner sets TEST_UNDECLARED_OUTPUTS_DIR; anything
    written there is preserved as a test artefact under
    `bazel-testlogs/.../test.outputs/`. No in-tree directory is ever created.
    """
    out = os.environ.get("TEST_UNDECLARED_OUTPUTS_DIR")
    if out:
        os.makedirs(out, exist_ok=True)
        return out
    fallback = "/tmp/coralnpu_per_op_cycles"
    os.makedirs(fallback, exist_ok=True)
    return fallback


@cocotb.test()
async def core_mini_rvv_mobilenetv1_a025_96_fft_int8_tf_flowers_per_op_cycles(
    dut,
):
    ...
    # Sanity: at least one non-zero cycle sample.
    ...
    # Also print a compact table to the test log for quick eyeballing.
