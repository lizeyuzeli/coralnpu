...
"""Cocotb test that runs the ST-AI-Zoo ResNet-8 32x32 INT8 CIFAR-10 (10-class)
classifier end-to-end on RvvCoreMini and compares the produced probability
vector against the BUILTIN_REF TFLite golden.

Input is uint8 [1, 32, 32, 3]; output is float32 [1, 10] post-DEQUANTIZE.
"""
...
_ELF = "run_resnet8_32_tfs_int8_cifar10_binary.elf"
_INPUT_NPY = "test_data/input_0.npy"
_EXPECTED_NPY = "test_data/expected_output_0.npy"

# Softmax LSB ~ 1/256; tolerance is comfortable headroom around it.
_MAX_ABS_DIFF = 3e-2


@cocotb.test()
async def core_mini_rvv_resnet8_32_tfs_int8_cifar10(dut):
    ...
    print("ResNet-8 INT8 CIFAR-10 inference matched reference.", flush=True)
