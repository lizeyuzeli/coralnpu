...
// "ResNet-8 32x32 tfs INT8" CIFAR-10 (10-class) classifier.
//
// Op inventory (8 distinct kinds):
//   CONV_2D x9, ADD x3, AVERAGE_POOL_2D x1, RESHAPE x1,
//   FULLY_CONNECTED x1, SOFTMAX x1, QUANTIZE x1, DEQUANTIZE x1.
//
// Boundary dtypes:
//   Input  : (1, 32, 32, 3) uint8 -- pixels in [0.0, 1.0] float-domain
//                                    (scale=1/255, zp=0).
//   Output : (1, 10)        float32 (trailing DEQUANTIZE).
//
// Heavy ops (CONV_2D / FULLY_CONNECTED) use the optimized RVV kernels;
// the residual ADD / AVG-POOL / RESHAPE / SOFTMAX / QUANTIZE / DEQUANTIZE
// fall back to the TFLM reference kernels.
