...
"""Cocotb test that runs the ST-AI-Zoo FD-MobileNet alpha=0.25 128x128 tfs
INT8 (5-class) tf_flowers classifier end-to-end on RvvCoreMini and compares
the produced probability vector against the BUILTIN_REF TFLite golden.

Input is uint8 [1, 128, 128, 3]; output is float32 [1, 5] post-DEQUANTIZE.
"""
...
_ELF = "run_fdmobilenet_a025_128_tfs_int8_tf_flowers_binary.elf"
_INPUT_NPY = "test_data/input_0.npy"
_EXPECTED_NPY = "test_data/expected_output_0.npy"

# Softmax output zp scale is 1/256 ~ 3.9e-3. With BUILTIN_REF golden the RVV
# device path should match to <= a few LSB; 3e-2 gives comfortable headroom.
_MAX_ABS_DIFF = 3e-2


@cocotb.test()
async def core_mini_rvv_fdmobilenet_a025_128_tfs_int8_tf_flowers(dut):
    ...
    print("FD-MobileNet 0.25@128 INT8 inference matched reference.",
          flush=True)
