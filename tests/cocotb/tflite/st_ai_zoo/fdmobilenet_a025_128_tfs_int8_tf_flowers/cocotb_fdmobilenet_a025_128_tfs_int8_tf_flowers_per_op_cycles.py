...
"""Cocotb test that runs the FD-MobileNet 0.25@128 tfs INT8 model with a
...
_RUNFILES_PREFIX = (
    ...
    "fdmobilenet_a025_128_tfs_int8_tf_flowers/"
)
_ELF = "run_fdmobilenet_a025_128_tfs_int8_tf_flowers_per_op_cycles_binary.elf"
_INPUT_NPY = "test_data/input_0.npy"

# Must match kMaxOps / kTagBytes in
# run_fdmobilenet_a025_128_tfs_int8_tf_flowers_per_op_cycles.cc.
_MAX_OPS = 64
_TAG_BYTES = 24
...
def _pick_reports_dir() -> str:
    ...
    if out:
        ...
        return out
    fallback = "/tmp/coralnpu_per_op_cycles_fdmobilenet"
    os.makedirs(fallback, exist_ok=True)
    return fallback


@cocotb.test()
async def core_mini_rvv_fdmobilenet_a025_128_tfs_int8_tf_flowers_per_op_cycles(
    dut,
):
    ...
    assert input_data.size == 1 * 128 * 128 * 3, (
        f"Unexpected input size {input_data.size}, want {1*128*128*3}")
    ...
    print(
        f"FD-MobileNet 0.25@128 INT8 total cycles: {cycle_count}", flush=True
    )
