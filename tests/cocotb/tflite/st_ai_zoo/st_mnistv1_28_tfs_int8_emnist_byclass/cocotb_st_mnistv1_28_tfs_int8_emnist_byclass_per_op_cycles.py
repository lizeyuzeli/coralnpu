...
"""Cocotb test that runs the ST MNIST-v1 28x28 INT8 EMNIST/byclass model with
a per-op MicroProfiler hooked into the interpreter and dumps a per-op
...
_ELF = "run_st_mnistv1_28_tfs_int8_emnist_byclass_per_op_cycles_binary.elf"
...
def _pick_reports_dir() -> str:
    ...
    if out:
        ...
        return out
    fallback = "/tmp/coralnpu_per_op_cycles_st_mnistv1"
    os.makedirs(fallback, exist_ok=True)
    return fallback


@cocotb.test()
async def core_mini_rvv_st_mnistv1_28_tfs_int8_emnist_byclass_per_op_cycles(
    dut,
):
