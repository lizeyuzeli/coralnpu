...
"""Cocotb test that runs the MiniResNet-V1 s1 64x50 INT8 model with a per-op
...
_ELF = "run_miniresnetv1_s1_64x50_tl_int8_per_op_cycles_binary.elf"
_INPUT_NPY = "test_data/input_0.npy"

# Must match kMaxOps / kTagBytes in
# run_miniresnetv1_s1_64x50_tl_int8_per_op_cycles.cc.
_MAX_OPS = 64
_TAG_BYTES = 24
...
def _pick_reports_dir() -> str:
    ...
    if out:
        ...
        return out
    fallback = "/tmp/coralnpu_per_op_cycles_miniresnet"
    os.makedirs(fallback, exist_ok=True)
    return fallback


@cocotb.test()
async def core_mini_rvv_miniresnetv1_s1_64x50_tl_int8_per_op_cycles(dut):
    ...
    print(
        f"MiniResNet-V1 s1 64x50 INT8 total cycles: {cycle_count}",
        flush=True,
    )
