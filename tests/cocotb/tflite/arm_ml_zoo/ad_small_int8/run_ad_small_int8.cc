...
// "Anomaly Detection MicroNet Small INT8" model.
//
// Operators (see definition.yaml): AVERAGE_POOL_2D, CONV_2D,
// DEPTHWISE_CONV_2D, RELU6, RESHAPE.
//
// Input shape  : (1, 32, 32, 1) int8 (32x32 mel spectrogram tile)
// Output shape : (1, 8)         int8 (raw logits across 8 machine IDs)
...
#include "tests/cocotb/tflite/arm_ml_zoo/ad_small_int8/ad_small_int8.h"

namespace {
using AdSmallOpResolver = tflite::MicroMutableOpResolver<5>;
using coralnpu_v2::opt::litert_micro::Register_CONV_2D;
using coralnpu_v2::opt::litert_micro::Register_DEPTHWISE_CONV_2D;

TfLiteStatus RegisterOps(AdSmallOpResolver& op_resolver) {
...
constexpr int kInputBytes = 1 * 32 * 32 * 1;  // int8 mel tile
constexpr int kOutputBytes = 1 * 8;           // int8 logits
}  // namespace
...
// MicroNet Small (~250KB model) needs working memory for the conv blocks;
// 1MB is comfortable.
...
int main(int argc, char** argv) {
  ...
  const tflite::Model* model = tflite::GetModel(g_ad_small_int8_model_data);
  if (model->version() != TFLITE_SCHEMA_VERSION) {
    std::strncpy(inference_status_message, "Bad model schema version", 31);
    return -1;
  }

  AdSmallOpResolver op_resolver;
