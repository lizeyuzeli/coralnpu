...
"""Cocotb test that runs the Arm ML-zoo Anomaly-Detection MicroNet Small INT8
...
_RUNFILES_PREFIX = "coralnpu_hw/tests/cocotb/tflite/arm_ml_zoo/ad_small_int8/"
_ELF = "run_ad_small_int8_binary.elf"
_INPUT_NPY = "test_data/input_0.npy"
_EXPECTED_NPY = "test_data/expected_output_0.npy"


@cocotb.test()
async def core_mini_rvv_ad_small_int8(dut):
    ...
    assert input_data.size == 32 * 32, (
        f"Unexpected input size {input_data.size}, want {32 * 32}")
    assert expected_output.size == 8, (
        f"Unexpected expected output size {expected_output.size}, want 8")
    ...
    # MicroNet Small has many 1x1/3x3 conv blocks; budget generously.
    cycle_count = await fixture.run_to_halt(timeout_cycles=2000_000_000)
    print(f"AD MicroNet Small INT8 total cycles: {cycle_count}", flush=True)
    ...
    print("AD MicroNet Small INT8 inference matched reference.", flush=True)
