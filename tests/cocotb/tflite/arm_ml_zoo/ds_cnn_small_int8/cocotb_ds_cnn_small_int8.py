...
"""Cocotb test that runs the Arm ML-zoo DS-CNN Small INT8 keyword-spotting
model end-to-end on the RvvCoreMini and checks the produced output against
...
_RUNFILES_PREFIX = "coralnpu_hw/tests/cocotb/tflite/arm_ml_zoo/ds_cnn_small_int8/"
_ELF = "run_ds_cnn_small_int8_binary.elf"
_INPUT_NPY = "test_data/input_0.npy"
_EXPECTED_NPY = "test_data/expected_output_0.npy"


@cocotb.test()
async def core_mini_rvv_ds_cnn_small_int8(dut):
    ...
    assert input_data.size == 490, (
        f"Unexpected input size {input_data.size}, want 490")
    ...
    # DS-CNN Small contains a non-1x1/3x3 stem Conv2D that still falls back to
    # the reference kernel, plus 4 depthwise + 4 pointwise blocks. Give it a
    # large but bounded cycle budget.
    cycle_count = await fixture.run_to_halt(timeout_cycles=400_000_000)
    ...
    print("DS-CNN Small INT8 inference matched reference output.", flush=True)
