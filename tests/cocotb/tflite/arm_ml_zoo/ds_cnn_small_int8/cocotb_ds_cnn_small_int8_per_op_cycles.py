...
"""Cocotb test that runs the Arm ML-zoo DS-CNN Small INT8 model with a
per-op MicroProfiler hooked into the interpreter and dumps a per-op
...
_RUNFILES_PREFIX = (
    "coralnpu_hw/tests/cocotb/tflite/arm_ml_zoo/ds_cnn_small_int8/"
)
_ELF = "run_ds_cnn_small_int8_per_op_cycles_binary.elf"
_INPUT_NPY = "test_data/input_0.npy"

# Must match kMaxOps / kTagBytes in run_ds_cnn_small_int8_per_op_cycles.cc.
_MAX_OPS = 64
_TAG_BYTES = 24
...
def _pick_reports_dir() -> str:
    ...
    if out:
        ...
        return out
    fallback = "/tmp/coralnpu_per_op_cycles_ds_cnn_small"
    os.makedirs(fallback, exist_ok=True)
    return fallback


@cocotb.test()
async def core_mini_rvv_ds_cnn_small_int8_per_op_cycles(dut):
    ...
    assert input_data.size == 1 * 490, (
        f"Unexpected input size {input_data.size}, want {1*490}")
    ...
    print(f"DS-CNN Small INT8 total cycles: {cycle_count}", flush=True)
