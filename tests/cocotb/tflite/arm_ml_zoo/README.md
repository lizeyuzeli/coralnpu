...
| Model | `.tflite` size | cycles | sim time (ns) | cycles / KB |
|---|---:|---:|---:|---:|
...
cycles are roughly `params x average feature-map area`.** The smallest
file (`ds_cnn_small_int8`) ends up in the same cycle ballpark as the
largest (`micronet_vww2_int8`), and the most expensive of all is the
111 KB `kws_micronet_small_int8`, because its 49x10 time-frequency map
is barely downsampled and every weight is multiplied hundreds of times.
