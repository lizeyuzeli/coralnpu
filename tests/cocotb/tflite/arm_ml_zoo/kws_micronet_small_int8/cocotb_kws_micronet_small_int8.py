...
"""Cocotb test that runs the Arm ML-zoo KWS MicroNet Small INT8 model
end-to-end on the RvvCoreMini and checks the produced logits against the
...
_RUNFILES_PREFIX = "coralnpu_hw/tests/cocotb/tflite/arm_ml_zoo/kws_micronet_small_int8/"
_ELF = "run_kws_micronet_small_int8_binary.elf"
_INPUT_NPY = "test_data/input_0.npy"
_EXPECTED_NPY = "test_data/expected_output_0.npy"


@cocotb.test()
async def core_mini_rvv_kws_micronet_small_int8(dut):
    ...
    assert input_data.size == 49 * 10, (
        f"Unexpected input size {input_data.size}, want {49 * 10}")
    ...
    print(f"KWS MicroNet Small INT8 total cycles: {cycle_count}", flush=True)
    ...
    print("KWS MicroNet Small INT8 inference matched reference.", flush=True)
